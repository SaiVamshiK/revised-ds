    ...
    while(k--)
    {
        ...
        for(int i=0;i<1024;i++)
        {
            if(main_arr[i]!=0)
            {
                if(set==1)
                {
                    int cur_freq=main_arr[i],cur_elem=i;
                    if(cur_freq%2!=0)
                    {
                        int replace=(cur_freq/2)+1;
                        a[i]-=(replace);
                        set=0;
                        int xorVal=((cur_elem)^(x));
                        a[xorVal]+=replace;
                    }else{
                        int replace=(cur_freq/2);
                        a[i]-=(replace);
                        int xorVal=((cur_elem)^(x));
                        a[xorVal]+=replace;
                    }
    ...
    while(k--)
    {
        ...
        for(int i=0;i<=1023;i++)
        {
            if(a[i]!=0)
            {
                ...
                {
                    int transfer=a[i]/2;
                    int pos_to_transfer=(x^i);
                    temp[i]+=a[i]-transfer;
                    temp[pos_to_transfer]=temp[pos_to_transfer]+transfer;
                }else{
                    int transfer=a[i]/2;
                    if(isLast)
                    {
