...
pi func(int *a,int i,int j,vector<vector<pi>> &dp)
{
	...
	for(int k=i;k<=(j-1);k++)
	{
		...
		if(dp[k+1][j].first==-1)
		{
			right=func(a,k+1,j,dp);
			dp[k+1][j]=right;
		}else{
			right=dp[k+1][j];
		}
		int smokeGenerated=(left.second*right.second)+left.first+right.first;
		int amountGenerated=((left.second+right.second)%100);
		if(minSmoke>smokeGenerated)
		{
			minSmoke=smokeGenerated;
			amount=amountGenerated;
		}
	}
	return dp[i][j]={minSmoke,amount};
}
int main()
{
    int n;
